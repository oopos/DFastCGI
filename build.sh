...
This script build DFastCGI library over a machine.
...
DOCDIR_PATH="${DESTDIR}/${PREFIX}/share/doc/DFastCGI"
INCLUDEDIR="${DESTDIR}/${PREFIX}/include/d/DFastCGI"
...
case ${DC} in
    ldc | ldc2)
        if [[ $SHARED_LIB -eq 1 ]]; then
            if [[ $VERBOSE -ge 2 ]]; then
                echo -e "\033[33mllvm-ld -link-as-library -o libDFastCGI.bc -lm -ldl -lrt -soname=DFastCGI *.bc\033[0;0m"
            fi
            llvm-ld -link-as-library -o libDFastCGI.bc -lm -ldl -lrt -soname=DFastCGI  $(find . -name "*.bc")
            if [[ $? -ge 1 ]]; then
                fail "llvm-ld"
            fi
            if [[ $VERBOSE -ge 2 ]]; then
                echo -e "\033[33mllc -relocation-model=pic libDFastCGI.bc\033[0;0m"
            fi
            llc -relocation-model=pic libDFastCGI.bc
            if [[ $? -ge 1 ]]; then
                fail "llc"
            fi
            if [[ $VERBOSE -ge 2 ]]; then
                echo -e "\033[33mgcc -shared libDFastCGI.s -o ${LIBDIR_PATH}/libDFastCGI-${COMPILER}.so\033[0;0m"
            fi
            gcc -shared -Wl,-soname,libDFastCGI-${COMPILER}.so libDFastCGI.s -o ${LIBDIR_PATH}/libDFastCGI-${COMPILER}.so
            if [[ $? -ge 1 ]]; then
                fail "gcc"
            fi
            if [ -e libDFastCGI.bc ]; then
                rm *.bc
            fi
            if [[ $? -ge 1 ]]; then
                fail "rm *.bc"
            ...
            if [[ $VERBOSE -ge 2 ]]; then
                echo -e "ar rcs ${LIBDIR_PATH}/libDFastCGI-${COMPILER}.a *.o"
            fi
            ar rcs ${LIBDIR_PATH}/libDFastCGI-${COMPILER}.a  $(find . -name "*.o")
            if [[ $? -ge 1 ]]; then
                fail "ar"
            fi
            if [[ $VERBOSE -ge 2 ]]; then
                echo -e "{LIBDIR_PATH}/libDFastCGI-${COMPILER}.a"
            fi
            ranlib ${LIBDIR_PATH}/libDFastCGI-${COMPILER}.a
            if [[ $? -ge 1 ]]; then
                fail "ranlib"
            fi
        fi
